...
c = rgb2lab(c);
c(:,:,2) = zeros(size(c,1), size(c,2));
c(:,:,3) = zeros(size(c,1), size(c,2));
c= lab2rgb(c);
...
%u(i_r_min:i_r_max,j_r_min:j_r_max,2) = collage(r(:,:,2),s(:,:,2),interieur);
%u(i_r_min:i_r_max,j_r_min:j_r_max,3) = collage(r(:,:,3),s(:,:,3),interieur);
u= lab2rgb(u);
%r= lab2rgb(r);
%s= lab2rgb(s);
hold off;
