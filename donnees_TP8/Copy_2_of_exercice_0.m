...
%each pixel gets coordinates (x,y) in [-0.5, 0.5]
[f_x,f_y] = meshgrid(1:nb_colonnes,1:nb_lignes);
f_x = f_x/nb_colonnes-0.5;
f_y = f_y/nb_lignes-0.5;

%getting the low frequencies, using a weight for eqch pixel depending on
%the distance to the center
%recuperation des basses frequences
masque = 1 ./ (1 + (f_x.^2 + f_y.^2) ./ eta);
...
subplot(2,3,3);
affichage(double(u)-u_barre,'$x$','$y$','Image $\overline{u}^c$');
