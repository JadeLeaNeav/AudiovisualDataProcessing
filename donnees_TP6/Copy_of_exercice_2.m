...
%%determiner Dpar segmentation avec r >= 204, g >= 204 b <= 51 
D = ( (u0(:, :, 1) >= 204) & (u0(:, :, 2) >= 204) & (u0(:, :, 3) <= 51));
...
while convergence > 1e-3
	...
	% Itération (6) :
    for i = 1:nb_canaux
	    u_kp1(:,i) = inpainting(b(:,i),u_k(:,i),lambda,Dx,Dy,epsilon, D);
    end 
	...
	% Affichage de l'image restaurée à chaque itération :
	subplot(1,2,2)
